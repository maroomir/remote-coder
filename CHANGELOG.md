...
- **Telegram 다국어(i18n)**: 사용자에게 보이는 문자열용 영어·한국어 카탈로그와 고급 설정(`/advanced`)의 언어 전환을 추가했습니다. 기본 인터페이스 언어는 영어입니다.

### 변경됨

- **`/help`·알림**: 에이전트·PLAN·ASK 관련 도움말과 알림 전달이 선택한 언어와 일치하도록 정리했습니다.
- **대화 맥락**: SQLite 대화 저장과 파서가 표시하는 맥락 라벨·포맷을 언어 설정에 맞게 출력합니다.
- **README**: 기본 언어가 영어이며 한국어는 고급 설정에서 전환한다는 안내를 명확히 했습니다.
