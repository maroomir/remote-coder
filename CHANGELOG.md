...
- **Recurring read-only scheduled jobs**: `/schedule <interval> <mode> <instruction>` registers a repeating read-only job (research, ask, or plan only — never agent/fix), and `/schedule` lists existing schedules with a confirmation-gated remove button. Schedules persist across restarts, fire on a single background thread, and never commit or push; a PLAN result still requires a manual Run-plan tap to write anything.
