...
텔레그램 메시지만으로 집 또는 사무실 컴퓨터의 Claude Code/Codex CLI를 실행해 코딩 작업을 자동 수행하고, 결과를 별도 Git 브랜치에 커밋한 뒤 사용자에게 알려주는 원격 AI 개발 자동화 시스템입니다.
...
- 여러 사용자가 동시에 사용하는 SaaS형 멀티테넌트 서비스는 초기 범위에서 제외합니다.
...
- 개인 개발자
- 로컬 머신에서 Claude Code/Codex CLI를 사용하는 개발자
...
5. 서버는 Claude Code 또는 Codex CLI를 비대화형 모드로 실행합니다.
...
- 시스템은 텔레그램 Bot API의 Webhook을 통해 메시지를 수신해야 합니다.
- 허용된 Telegram User ID 또는 Chat ID의 요청만 처리해야 합니다.
...
- Claude Code와 Codex CLI를 지원해야 합니다.
- `/model claude`, `/model codex` 명령으로 기본 모델을 변경할 수 있어야 합니다.
...
- 작업 완료 시 다음 정보를 전송해야 합니다.
  ...
  - 수정 파일 목록
  - 오류 요약
- 실패 시 에러 로그의 핵심 메시지를 요약해서 전송해야 합니다.
...
| `/branches` | 기본 프로젝트 저장소의 로컬·원격 브랜치 목록 조회 |
| `/branch` / `/branch <이름>` | 기본 프로젝트 현재 브랜치 조회 / 로컬 브랜치가 있으면 `git switch` |
| `/rebase` / `/rebase <branch>` | 기본 프로젝트에서 브랜치를 main 기준 rebase 후 main에 fast-forward 병합·push |
| `/clear branch` | 확인(`y`/`Y`) 후 등록 프로젝트의 `remote-*` 로컬·원격 브랜치 및 연결 worktree 일괄 삭제 |
| `/clear memory` | 확인(`y`/`Y`) 후 대화 기억 SQLite 데이터베이스 초기화 |
| `/monitor model` | 현재 채팅의 적용 프로젝트와 무관하게, 채팅 기본 모델 기준 CLI 인증/버전 조회(구독·남은 할당량은 Anthropic/OpenAI 안내 링크·대시보드 참고) |
| `/monitor memory` | 현재 채팅·적용 프로젝트 기준 SQLite 대화 기억 행 수·역할별 행 수·DB 파일 크기 |
| `/monitor branch` | 적용 프로젝트 저장소의 현재 브랜치·로컬/원격 브랜치 수 및 목록 요약(`/branches` 대체 후보) |
...
- Telegram User ID/Chat ID allowlist를 반드시 적용합니다.
- Bot Token, API Key, 프로젝트 경로 등 민감 정보는 환경 변수 또는 `.env` 파일로 관리합니다.
...
- AI 엔진은 인터페이스 형태로 추상화하여 Claude/Codex 외 도구를 추가할 수 있어야 합니다.
...
| 모듈 | 역할 |
|---|---|
| Webhook Controller | 텔레그램 Webhook 수신 및 기본 검증 |
| Auth Service | 허용 사용자 확인 |
| Command Parser | 자연어/명령어를 작업 요청으로 변환 |
| Job Manager | 작업 생성, 상태 변경, 백그라운드 실행 관리 |
| Git Service | worktree 생성, 브랜치 생성, diff 확인, 커밋 |
| AI Runner | Claude/Codex 실행 추상화 |
...
| 동시 작업 충돌 | 같은 프로젝트에 여러 요청이 들어올 수 있음 | 프로젝트별 lock 또는 작업 큐 도입 |
