...
- 자연어 메시지는 파싱 후 즉시 실행하지 않고, 현재 프로젝트·작업 브랜치·사용 모델(및 모드)을 표시한 뒤 `y` 또는 `Y`(또는 고급 설정 시 인라인 확인 버튼)를 받아야 Job으로 생성합니다. `plan:`/`ask:`/`계획:`/`질문:` 접두(ASCII 대소문자 무시, 콜론은 `:` 또는 `：`) 또는 `/plan`/`/ask`로 시작하면 **읽기 전용** plan/ask 모드로 분류되며, 확인 절차는 agent와 동일하지만 실행 시 커밋·push는 하지 않고 detached worktree에서만 동작합니다. 본문 없이 `/plan` 또는 `/ask`만 입력하면 다음 메시지 1건을 해당 모드의 작업 지시문으로 받아 동일한 확인 절차를 진행해야 합니다. PLAN 모드는 계획 확정 전에 사용자 결정이 필요하면 모델이 구조화된 `plan-decisions` 블록(질문 최대 3개·옵션 2~4개)을 출력하고, 봇이 이를 인라인 버튼으로 한 번에 하나씩 물은 뒤 선택을 2차 PLAN 실행에 주입(detached worktree 재실행, 컨텍스트 주입)하여 최종 계획을 만듭니다. 결정이 없으면 1차 출력을 그대로 전달하며, 모든 모델에서 동일하게 동작합니다.
