...
| `/projects` | 등록된 프로젝트 목록 및 이 채팅의 적용 프로젝트 조회 |
| `/init` | 이 채팅의 작업 프로젝트·기본 모델·`/clear` 확인 대기 상태를 서버 시작 직후처럼 초기화(적용 프로젝트는 등록 파일 기본값으로 폴백) |
| `/reports` | 현재 채팅·현재 프로젝트 기준으로 SQLite 대화 기억 요약 조회 |
| `/branch` / `/branch <이름>` | 이 채팅 적용 프로젝트의 현재 브랜치 조회 / 로컬 브랜치가 있으면 `git switch` |
| `/rebase` / `/rebase <branch>` | 적용 프로젝트에서 브랜치를 main 기준 rebase 후 main에 fast-forward 병합·push |
| `/clear branch` | 확인(`y`/`Y`) 후 등록 프로젝트의 `remote-*` 로컬·원격 브랜치 및 연결 worktree 일괄 삭제 |
| `/clear memory` | 확인(`y`/`Y`) 후 대화 기억 SQLite 데이터베이스 초기화 |
| `/monitor model` | 이 채팅 기본 모델 기준 CLI 인증/버전 조회(구독·남은 할당량은 Anthropic/OpenAI 안내 링크·대시보드 참고) |
