...
텔레그램 메시지만으로 집 또는 사무실 컴퓨터의 Claude Code/Codex/Gemini CLI 또는 Ollama 로컬 모델을 실행해 코딩 작업을 자동 수행하고, 결과를 별도 Git 브랜치에 커밋한 뒤 사용자에게 알려주는 원격 AI 개발 자동화 시스템입니다.
...
- Claude Code, Codex CLI, Gemini CLI, Ollama 중 원하는 엔진을 선택해 실행할 수 있습니다.
...
- 개인 개발자
- 로컬 머신에서 Claude Code/Codex/Gemini CLI 또는 Ollama 로컬 모델을 사용하는 개발자
...
5. 서버는 Claude Code, Codex CLI, Gemini CLI 또는 Ollama를 비대화형 모드로 실행합니다.
...
- 허용된 사용자만 명령 실행
- 기본 명령 파싱
- 기본 모델 선택: Claude, Codex, Gemini 또는 Ollama
...
- Ollama는 로컬 HTTP API(`/api/chat`, `/api/tags`)를 기본으로 사용합니다.
- `/model ollama`는 로컬에 탑재된 모델을 조회해 변경 대상 목록을 보여줘야 합니다.
- 세부 모델이 지정되지 않은 경우 `REMOTE_CODER_OLLAMA_DEFAULT_MODEL` 또는 첫 번째 로컬 모델을 사용할 수 있습니다.
- PLAN, ASK, RESEARCH 모드는 읽기 전용 prompt로 실행합니다.
- AGENT, FIX 모드는 상업용 CLI와 완전 동일한 파일 도구를 보장하지 않습니다. Ollama adapter는 fenced unified diff patch를 요청하고 유효한 patch를 `git apply`로 적용하는 best-effort 방식으로 지원합니다.
- reply 세션은 `REMOTE_CODER_HOME/ollama_sessions/` transcript를 사용해 최근 user/assistant 메시지를 재주입합니다.
- 사용량은 provider quota가 아니라 Ollama 응답의 prompt/eval token count와 로컬 transcript 기준으로 모니터링합니다.
...
- 프로젝트에 선택적 `test_command`가 설정된 경우, AGENT/FIX 작업은 러너 성공 후 worktree에서 해당 명령을 실행해 **통과할 때만 커밋**합니다(보수적 커밋). 실패 시 변경을 미커밋 상태로 보존하고 worktree를 유지하며 검증 출력을 결과에 포함합니다. `test_command` 미설정 시 기존 "항상 커밋" 동작을 유지합니다(opt-in, 기본 OFF). 관리 UI(`/projects`)에서 설정합니다.
...
| `/model ollama` | 기본 모델을 Ollama로 변경하고 로컬 모델 선택 |
...
| `/pr` / `/pr <branch>` | 현재 프로젝트·채팅에서 커밋된 성공 Job 브랜치 중 설정 원격에 남아 있는 브랜치 인라인 선택 / 동일한 Job 소유권·커밋 여부·원격 존재 여부를 재검증한 뒤 `gh`로 main/master 대상 GitHub PR 생성 또는 기존 PR URL 조회 |
...
| `/schedule` / `/schedule <간격> <모드> <지시문>` | 현재 프로젝트·채팅의 반복 스케줄 조회(확인 버튼이 붙은 제거 버튼 포함) / 반복 **읽기 전용** 작업 등록(모드는 research·ask·plan 전용, 간격은 30m·6h·1d 형식). 스케줄은 SQLite에 영속되며 커밋·push를 하지 않습니다. |
...
- AI CLI 실행과 Ollama patch 적용 시 위험 명령/파일 변경 가능성이 있으므로 허용 프로젝트 경로를 제한합니다.
...
- 프로세스 실패, Git 실패, CLI/API 실패를 구분해서 기록해야 합니다.

### N3. 확장성

- AI 엔진은 인터페이스 형태로 추상화하여 Claude/Codex/Gemini/Ollama 외 도구를 추가할 수 있어야 합니다.
...
| AI Runner | Claude/Codex/Gemini/Ollama 실행 추상화 |
...
3. GitHub PR 자동화
   - `/pr` 명령을 안정화해 커밋된 성공 Job 브랜치에서 PR을 만들거나 기존 PR URL을 반환합니다.
