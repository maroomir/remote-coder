...
텔레그램 인라인 버튼은 반복 입력을 줄이기 위해 선택형 명령에 우선 적용합니다. `/model`, `/status`, `/project`, `/branch`, `/rebase`, `/stop`은 인자 없이 호출하면 선택 가능한 항목을 인라인 버튼으로 보여주고, 버튼 선택은 기존 슬래시 명령 실행 경로를 재사용합니다.
...
| `/project` / `/project <이름>` | 작업 프로젝트 인라인 선택 / 작업 프로젝트 전환 |
...
| `/rebase` / `/rebase <branch>` | main/master를 제외한 로컬 브랜치 인라인 선택 / 적용 프로젝트에서 브랜치를 main 기준 rebase 후 main에 fast-forward 병합·push |
...
| `/monitor model` | 이 채팅 기본 모델 기준 CLI 인증/버전 조회 및 최근 Job 로그에서 관측된 세부 모델명·토큰 사용량 요약(구독·남은 할당량은 Anthropic/OpenAI/Google 안내 링크·대시보드 참고) |
