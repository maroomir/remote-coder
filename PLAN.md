...
- 자연어 메시지는 파싱 후 즉시 실행하지 않고, 현재 프로젝트·작업 브랜치·사용 모델을 표시한 뒤 `y` 또는 `Y` 확인을 받아야 Job으로 생성합니다. (메시지가 `plan:`/`ask:`/`계획:`/`질문:` 접두(ASCII 대소문자 무시, 콜론은 `:` 또는 `：`) 또는 `/plan`/`/ask` 명령으로 시작하면 **읽기 전용** plan/ask 모드로 분류되며, 이 경우 확인 단계·브랜치·커밋·push 없이 detached worktree에서만 실행합니다.)
