...
def create_webhook_router(
    ...
    def _handle_callback_query(
        ...
        if cq.data == _CLOSE_PANEL:
            ...
            return {"status": "ok"}
        if cq.data.startswith(f"{PLAN_EXECUTE_CALLBACK_PREFIX}:"):
            return _handle_plan_execute(
    ...
    def _handle_plan_execute(
        ...
        if (
            ...
            return {"status": "ignored"}
        with executed_plan_jobs_lock:
            already_started = job_id in executed_plan_jobs
            if not already_started:
                executed_plan_jobs.add(job_id)
        if already_started:
