...
| `/log [job_id]` | 최근 Job을 선택하거나 특정 Job의 AI 출력 로그 확인 |
...
| `/pr [branch]` | 커밋된 성공 Job 브랜치에서 `gh`로 GitHub PR 생성 |
...
브랜치 없이 `/pr`를 실행하면 현재 프로젝트와 Telegram 채팅에서 커밋된 성공 Job이 만든 브랜치 중 설정된 Git 원격에 남아 있는 브랜치만 표시합니다. `/pr <branch>` 직접 호출도 같은 소유 범위를 검사하고 원격 브랜치가 아직 존재하는지 다시 확인합니다. 이 명령을 사용하기 전에 [GitHub CLI](https://cli.github.com/)를 설치하고 `gh auth login`을 실행하세요.
