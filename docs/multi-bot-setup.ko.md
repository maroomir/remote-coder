...
*English: [multi-bot-setup.md](multi-bot-setup.md) · 한국어: 이 문서*
...
1. 서버를 띄운 뒤 같은 머신에서 `http://127.0.0.1:8000/` 로 이동합니다. 등록된 프로젝트가 없으면 대시보드에 **간편 설정** 위저드가 자동으로 표시됩니다(또는 우측 상단의 **간편 설정** 버튼을 누릅니다).
2. **봇 토큰**을 붙여넣고 **토큰 확인**을 누릅니다. 서버가 Telegram(`getMe`)으로 토큰을 검증합니다.
3. Telegram에서 새 봇과의 채팅을 열고 아무 메시지나 보내면, 위저드가 **Chat ID**를 자동으로 감지합니다(`getUpdates`). 직접 입력도 가능합니다.
4. **프로젝트 이름**, **저장소 루트 경로**, **기본 모델**을 입력하고 **프로젝트 생성**을 누르면, 재시작 없이 봇이 연결됩니다.

> 자동 채팅 감지는 `getUpdates`를 사용하는데, Telegram은 webhook이 설정되어 있으면 이를 비활성화합니다. 최초 설정 시에는 아직 webhook이 등록되지 않아 감지가 동작합니다. 실패하는 경우(예: 이미 webhook이 활성화됨) Chat ID를 직접 입력하세요.

### 전체 폼

여러 개의 Chat ID, 허용 User ID, webhook secret이 필요하면 **프로젝트 등록** 페이지(`http://127.0.0.1:8000/projects`)를 사용하세요.

1. **프로젝트 이름**, **저장소 루트 경로**, **기본 모델**을 입력합니다.
2. 해당 봇의 **bot_token**, (선택) **webhook_secret**, **허용 Chat ID**(최소 1개), (선택) **허용 User ID**를 입력합니다. 프로젝트 생성 시 secret을 비우면 관리 API가 프로젝트마다 고유한 256비트 URL-safe 값을 자동 생성합니다.
3. 저장하면 서버는 재시작 없이 해당 봇 인스턴스를 등록합니다.
