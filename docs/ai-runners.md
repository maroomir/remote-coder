...
Remote AI Coder runs Claude Code, Codex CLI, and Gemini CLI through the same Job flow. Whichever model you use, the CLI must be installed, authenticated, and available on `PATH` for the **same OS user account** that runs the server.

## Common Checks

1. Run `remote-coder doctor` to check local tool availability.
2. Smoke-test the CLI directly before using it through Telegram.
3. Select the default model with `/model`, or pass `model: claude`, `model: codex`, or `model: gemini` in a request.
...
- Check `command -v claude`, `command -v codex`, and `command -v gemini` as the server user.
