...
- 프로젝트 메타데이터·봇 토큰·allowlist는 로컬 레지스트리 파일(`projects.json`)에 저장됩니다. 기본 경로는 `PROJECT_ROOT/.remote-coder/projects.json`입니다.
...
`GET /api/projects` 응답에는 봇별 **`webhook_path`**(예: `/telegram/webhook/<sha256_hex>`)가 포함됩니다. 전체 webhook URL은 `<공개 HTTPS Base>` + `webhook_path` 입니다.

### Webhook URL과 토큰 해시

Webhook 경로의 마지막 세그먼트는 봇 토큰 문자열의 **SHA-256 16진 문자열 전체**입니다. BotFather에서 토큰을 재발급하면 해시가 바뀌므로 **Webhook URL도 다시 등록**해야 합니다.

## 3. Webhook 등록

공개 Base URL만 넘기면, 활성화된 모든 프로젝트에 대해 `setWebhook`을 호출합니다.
...
1. `.env`의 `DEFAULT_PROJECT`, `PROJECT_ROOT`, `WORKTREE_BASE_DIR`와 선택적 `TELEGRAM_*`로 첫 부팅 시 `projects.json`이 비어 있으면 한 레코드가 시드될 수 있습니다.
